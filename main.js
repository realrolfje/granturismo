...
function computeDriverStandings({ raceResults = {}, pointsRules = {}, drivers = [] }) {
  ...
  (raceResults.results || []).forEach((race) => {
    (race.finishers || []).forEach((finisher, index) => {
      ...
      entry.races += 1;
      entry.bestFinish = Math.min(entry.bestFinish, position ?? Infinity);
      if (position === 1) {
        entry.wins += 1;
      }
...
function renderStandings({ driverStandings, teamStandings, drivers }) {
...
async function init() {
  try {
    ...
    renderStandings({ driverStandings, teamStandings, drivers: data.teams.drivers });
